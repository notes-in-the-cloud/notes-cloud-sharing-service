...
```json
{
  ...
  "url": "http://localhost:8083/api/v1/share-links/4uJnzYqu0jtJithJii_s0WXo9G0Ykvx3dvZas7bcy4E",
...
Validates the token and returns the shared note.
...
SHARING_PUBLIC_BASE_URL=http://sharing-service:8083
```
...
SHARING_PUBLIC_BASE_URL=http://localhost:8083
```
...
spring.datasource.username=${POSTGRES_USER:notes_cloud_user}
spring.datasource.password=${POSTGRES_PASSWORD:notes_cloud_password}
...
sharing.public-base-url=${SHARING_PUBLIC_BASE_URL:http://localhost:8083}
...
```http
POST http://localhost:8083/api/v1/users/{userId}/notes/{noteId}/share-links
```

Open the returned link:

```http
GET http://localhost:8083/api/v1/share-links/{token}
```
...
- `http://sharing-service:8083` works only inside Kubernetes.
- For browser or Postman testing from your machine, use `http://localhost:8083` with port-forwarding.
- For production, `SHARING_PUBLIC_BASE_URL` should be a real public domain.
